...
exports.init = params => {
  ...
  var options = {
    uri:
      "https://api.github.com/repos/OPEnSLab-OSU/Loom/branches/" +
      params.branch,
    headers: {
...
exports.buildTree = () => {
  var options = {
    uri:
      "https://api.github.com/repos/OPEnSLab-OSU/Loom/git/trees/" +
      loom_github.tree_sha +
      "?recursive=1",
    headers: {
      "User-Agent": "Loom Configurator App"
    },
...
exports.getFile = params => {
  ...
  var options = {
    uri:
      "https://api.github.com/repos/OPEnSLab-OSU/Loom/contents/Loom/src/" +
      params.file,
    headers: {
      "User-Agent": "Loom Configurator App"
    },
  ...
  return loom_github.getContents(options).then(loom_github.findFile);
...
function parse_params(files) {
  ...
  for (file of files) {
    // for each file find param tags and build json
    promises.push(_find_param_tags(file).then(build_module_json));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var line of lines) {
      if (line.search("///") > 0) {
        ...
        var param_line =
          line.search("\\param") > 0
            ? line.substr(line.search("param")).trim()
            : null;
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var param of param_array) {
      if (param[0] == "module_name" || param[0] == "device_name") {
        var module_name = param[2].substring(
          param[2].indexOf('<"') + 2,
          param[2].indexOf('">')
        );
...
exports.parse = function(dir, callback) {
  ...
  read_header_files(dir)
    .then(parse_params, err => {
      console.log(err);
    }) // then for each header file found, look for and parse all '/param' values
    // .then( del_undef_elems, (err) => { console.log(err) }) // then remove any parameters with undefined descriptions
    .then(addToMaster, err => {
      console.log(err);
    }) // then add the resulting json_data to the master_json
    .then(
      master => {
